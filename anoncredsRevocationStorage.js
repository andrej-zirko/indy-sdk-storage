...
function prepareStorageWalletConfig(originalWalletConfig) {
    return Object.assign(originalWalletConfig, config.storage.walletConfig)
}

function prepareStorageWalletCredentials(originalWalletCredentials) {
    return Object.assign(originalWalletCredentials, config.storage.walletCredentials)
}

function initStorage() {
    const libraryEntryPoint = {};
    libraryEntryPoint[config.storage.entrypoint] = [ 'int', [ ] ];
    const storageLib = ffi.Library(config.storage.lib, libraryEntryPoint)
    
    storageLib[config.storage.entrypoint]()
}
...
async function run() {
    ...
    // When running this, add storage library on LD_LIBRARY_PATH
    // Consider using wallet names in lowercase
    // There might be an issue with mixed case wallets. 
    // In case of postgres storage plugin, it creates DB with lowercase and then tries to connect with mixedCase.
    initStorage()
    ...
    const issuerWalletConfig = prepareStorageWalletConfig({"id": "issuer_wallet"})
    const issuerWalletCredentials = prepareStorageWalletCredentials({"key": "issuer_wallet_key"})
    ...
    const proverWalletConfig = prepareStorageWalletConfig({"id": "prover_wallet"})
    const proverWalletCredentials = prepareStorageWalletCredentials({"key": "issuer_wallet_key"})
    ...
    const tailsWriterConfig = {'base_dir': pathToIndyClientHome() + "/tails", 'uri_pattern': ''}
    ...
    log("You can check your wallet structure in Storage now. Press any key to continue. Wallets will be deleted...")
    await keypress()
